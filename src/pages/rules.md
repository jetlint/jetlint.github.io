...
title: Rules
description: All 67 jetlint rules — 61 typescript-eslint ports plus 6 ESLint-core ports.
---

# Rules

jetlint ships **67 rules** in two families:

- **61 typescript-eslint ports** — type-aware rules verified against
  typescript-eslint's published test fixtures.
- **6 ESLint-core ports** — non-type-aware correctness rules
  (`no-dupe-keys`, `no-duplicate-case`, `no-self-assign`, `no-self-compare`,
  `use-isnan`, `valid-typeof`) that catch bugs the TypeScript checker
  cannot.
